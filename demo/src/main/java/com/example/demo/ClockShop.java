...
public class ClockShop implements Iterable<Time>,Serializable {
        ...
        public void toCheck_ID(){int i=0;
        for(Time cl:clocks){cl.toSet_ID(i++);}}
        public void remove(Time c){
           if(Fabric.flag==false){ try {
        ...
        for (Time s : clocks) {
            if (!uniqueMarks.contains(s.toGet_Mark())) {
                uniqueMarks.add(s.toGet_Mark());
            }
        }
